...
export function HomePage({ state, onChange, onExplore }: HomePageProps) {
  ...
  const periodLabel =
    ...
    state.period
  const headline = requireEstimateForLevel(
    state.level,
    "ARG",
    state.period,
    state.universe,
  ...
  const indigence = requireEstimateForLevel(
    state.level,
    "ARG",
    state.period,
    state.universe,
  ...
  return (
    <div className="overflow-clip">
      <section className="mx-auto grid min-h-[calc(100svh-8rem)] max-w-7xl items-center gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[1.12fr_0.88fr] lg:py-24">
        <div>
          ...
          <p className="mt-7 max-w-2xl text-lg leading-8 text-slate-600">
            Una publicación interactiva para pasar de la cifra nacional a las diferencias territoriales sin perder de vista qué se mide, de dónde viene cada dato y cuáles son sus límites.
          </p>
      ...
      <div className="mx-auto max-w-7xl px-5 pb-10 pt-4 sm:px-8 lg:pb-16">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-3 font-serif text-4xl font-semibold leading-tight tracking-[-0.03em] text-slate-950 sm:text-5xl">
            Después de la tendencia nacional, el país se abre territorio por territorio.
          </h2>
      ...
      {state.level === "province_2010" ? (
        <EditorialTerritoryStory state={state} onChange={onChange} onExplore={onExplore} />
      ) : (
        <section className="border-y border-slate-900/10 bg-[#e9eee9]">
          <div className="mx-auto grid max-w-7xl gap-5 px-5 py-16 sm:px-8 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-sky-950">
                Nivel departamental
              </p>
              <h3 className="mt-3 font-serif text-4xl font-semibold tracking-[-0.025em]">
                525 territorios, una identidad exacta.
              </h3>
              <p className="mt-4 max-w-2xl leading-7 text-slate-600">
                La narrativa editorial de portada sigue siendo provincial. El explorador departamental usa su release y transporte propios, con filtro por provincia y códigos de cinco dígitos.
              </p>
            </div>
            <Button onClick={onExplore}>Explorar departamentos →</Button>
          </div>
        </section>
      )}
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-5 py-16 sm:px-8 lg:grid-cols-[0.78fr_1.22fr] lg:items-center lg:py-24">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-950">Un territorio en contexto</p>
          ...
          <Button className="mt-6" variant="secondary" onClick={onExplore}>Ver detalle territorial →</Button>
        </div>

        <div className="rounded-[2rem] border border-slate-900/10 bg-white/55 p-6 sm:p-8">
          {selectedGeography && selectedValue !== null ? (
            <>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Selección actual · {selectedGeography.id}</p>
              <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
                <div>
                  <h3 className="font-serif text-3xl font-semibold">{selectedGeography.name}</h3>
            ...
            <>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Sin selección territorial</p>
              <h3 className="mt-3 font-serif text-3xl font-semibold">El mapa y la tabla comparten una sola selección.</h3>
              <p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">
                Elegí un territorio en el mapa o entrá al explorador. El identificador queda en la URL para que la vista pueda compartirse y recuperarse.
              </p>
            </>
          )}
        </div>
